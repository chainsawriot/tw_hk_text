...
parsed_text <- spacy_parse("Ms. Tsai attacks us.")

parsed_text <- spacy_parse("Ms. Tsai is being attacked.", dependency = TRUE)

entity_extract(parsed_text)

#### extraction of all sentences with Tsai, but not Tsai administrator
# 
...
tsai_sentences <- tokens(tsai_articles$content, what = 'sentence') %>% 
    unlist %>% str_subset("Tsai") %>% 
    str_subset("Tsai administration", negate = TRUE)

### Get a random sample of 50 sentences (weighted by wc)
# set.seed(12121)
...
tsai_random_sentences <- rio::import('tsai_sample.csv')

### Let's create the gold standard!

goldstandard <- rio::import()

tsai_random_sentences$gold <- 

tsai_random_sentences$tsai_sentences %>% corpus %>% 
    dfm(dictionary = data_dictionary_LSD2015) %>% 
    convert(to = 'data.frame') %>% mutate(wc = tsai_random_sentences$wc) %>% 
    mutate(nettone = ((positive - neg_positive) / wc) - ((negative - neg_negative) / wc))
...
extract_associated_word_sent(sent = tsai_random_sentences$tsai_sentences[1],
                             kw = "Tsai", attributive = FALSE)

tsai_verbs <- map(tsai_random_sentences$tsai_sentences, 
                  extract_associated_word_sent, kw = "Tsai")

tsai_verbs_att <- map(tsai_random_sentences$tsai_sentences, 
                      extract_associated_word_sent, kw = "Tsai", 
                      attributive = TRUE)

tsai_verbs %>% map_chr(paste, collapse = " ") %>% 
    dfm(dictionary = data_dictionary_LSD2015) %>% 
    convert(to = "data.frame")

tsai_verbs_att %>% map_chr(paste, collapse = " ") %>% 
    dfm(dictionary = data_dictionary_LSD2015) %>% 
    convert(to = "data.frame")

### Validate which one is better.
